-- luatz timetable
-- http://github.com/daurnimator/luatz
...
function timetable_methods:time ( )
	local year, month, day, hour, min, fsec = self:unpack()
	local sec = borrow(fsec, 0, 1000)
	return strformat ( "%02u:%02u:%02d", hour, min, sec )
end

function timetable_methods:date ( )
	local year, month, day = self:unpack()
	return strformat ( "%02u/%02u/%04d", day, month, year )
end
